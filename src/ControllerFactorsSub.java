...
public class ControllerFactorsSub extends Controller {
	...
	private Parent root;
	@FXML
	private CheckBox allowNegative;
	@FXML
...
public void goToSubProblem(ActionEvent event) throws IOException {
	negative = true;
		if (allowNegative.isSelected()) {
			negative = false;
		} else {
			negative = true;
		}
		...
		FXMLLoader loader = new FXMLLoader(getClass().getResource("subProblem.fxml"));
		root = loader.load();
		
		ControllerSub ControllerSub = loader.getController();

		ControllerSub.setDigit(digits1, digits2, negative);
		stage = (Stage)((Node)event.getSource()).getScene().getWindow();
		scene = new Scene(root);
		stage.setScene(scene);
		stage.show();
